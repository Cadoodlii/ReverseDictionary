...
This script visualizes three embedding spaces side-by-side for the same random hypernym groups:
1. StudentModel (text-only) embeddings
2. TeacherModel (multimodal text+image) embeddings
3. GloVe (ground-truth) embeddings
    ...
    with torch.no_grad():
        for entry in entries:
            ...
            if model_type.lower() == 'student':
                ...
                # For teacher model, find the corresponding image
                image_files = []
...
def visualize_all_embeddings(
    ...
    teacher_model_path: str = './ckpts/teacher_model.pt',
    ...
    """Visualize StudentModel, TeacherModel, and GloVe embeddings side-by-side."""
    ...
    # Get embeddings from all three sources
    ...
    print('Computing TeacherModel embeddings...')
    teacher_embeddings = get_model_embeddings(all_entries, teacher_model, teacher_ds, device, model_type='teacher')
    ...
    combined = np.vstack([student_embeddings, teacher_embeddings, glove_embeddings])
    tsne = TSNE(n_components=2, perplexity=perplexity, random_state=42, max_iter=1000)
    combined_2d = tsne.fit_transform(combined)
    
    n = len(all_entries)
    student_2d = combined_2d[:n]
    teacher_2d = combined_2d[n:2*n]
    glove_2d = combined_2d[2*n:]
    ...
    # TeacherModel embeddings
    axes[1].scatter(teacher_2d[:, 0], teacher_2d[:, 1], c=point_colors, s=50, alpha=0.6, edgecolors='k', linewidth=0.5)
    axes[1].set_title(f'Teacher Model (Text + Image)\n{k_hypernyms} hypernym groups', fontsize=14, fontweight='bold')
    axes[1].set_xlabel('t-SNE dimension 1')
    axes[1].set_ylabel('t-SNE dimension 2')
    axes[1].grid(True, alpha=0.3)
    
    # GloVe embeddings
    axes[2].scatter(glove_2d[:, 0], glove_2d[:, 1], c=point_colors, s=50, alpha=0.6, edgecolors='k', linewidth=0.5)
    axes[2].set_title(f'GloVe (Ground Truth)\n{k_hypernyms} hypernym groups', fontsize=14, fontweight='bold')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Visualize student, teacher, and GloVe embeddings side-by-side for hypernym groups')
    ...
    parser.add_argument('--teacher-model', default='./ckpts/teacher_model.pt', help='Path to teacher model checkpoint')
